...
let play_note param_list () =
  ...
  Midi.(write_output dev [ message_on ~note:note ~timestamp:0l ~volume:volume ~channel () ]);

  (* Sleep for 2 seconds  *)
  Unix.sleep 2;

  (* Send the note_off signal *)
  Midi.(write_output dev [ message_off ~note:note ~timestamp:0l ~volume:volume ~channel () ]);
